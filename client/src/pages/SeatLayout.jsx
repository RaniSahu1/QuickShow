  ...
  const bookTickets = async () => {
    try {
      if(!user) { toast.error("Please login to book tickets"); return;}
        if(!selectedTime || !selectedSeats.length) return toast.error("Please select a time and seats to book")
  ...
  return  show ? (
    <div className='flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-30 md:pt-50'>
      ...
      <div className='relative flex-1 flex flex-col items-center max-md:mt-16'>
       ...
       <button onClick={bookTickets} className='flex items-center gap-1 mt-20 py-3 px-10 text-sm bg-primary hover:bg-primary-dull transition rounded-full font-medium cursor-pointer active:scale-95'>Proceed to checkout
